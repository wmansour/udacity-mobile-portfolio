...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    // took these lines out of for loop because they do not need to be reassigned every time
...
function updatePositions() {
  ...
  var phaseArray = [];
	// phase calculations are now outside for loop for all items so we don't do it every time
	for (var j = 0; j < 5; j++) {
		phaseArray.push(Math.sin(top + j));
	}
...
//Limits the number of pizzas created
var rows = window.screen.height / 256;
var columns = window.screen.width / 256;
var pizzaNumber = columns * rows;

// Generates the sliding pizzas when the page loads
window.onload = function() {
  var columns = 8
  var elem;
  // Changed max # of pizzas because there will never be a need for 200
  ...
  for (var i = 0; i < pizzaNumber; i++) {
    ...
    elem.basicLeft = (i % columns) * 256;
    elem.style.top = (Math.floor(i / columns) * 256) + 'px';
